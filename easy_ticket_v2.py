...
class Ticket:
    ...
    def apiTrain(self, param):
        ...
        for item in response.json().get('data').get('result'):
            ret.append(self.paramatTrain(item, station_map))
        return ret
    '''
    {
    ...
    def paramatTrain(self, train, station_map):
        info = train.split('|')
        return {
            'xcode1' : info[0],
            'state' : info[1], # 状态 预定
            ...
            #'train_start' : station_map[info[4]], # 列车始发站
            #'train_end' : station_map[info[5]], # 列车终点站
            #'search_start' : station_map[info[6]], # 购买起点站
            #'search_end' : station_map[info[7]], # 购买到达站
            
            'train_start':station_map[info[4]] if info[4] in station_map else info[4], # 始发站
            'train_end':station_map[info[5]] if info[5] in station_map else info[5], # 终点站
            'search_start':station_map[info[6]] if info[6] in station_map else info[6], # 购买起点站
            'search_end':station_map[info[7]] if info[7] in station_map else info[7], # 购买到达站
